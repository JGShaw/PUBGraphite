...
describe MetricExtractor do
  ...
  context '#call_api' do 
    ...
    it 'gets the correct players' do
      VCR.use_cassette('player_api_call') do
        extractor = MetricExtractor.new(ENV['PUBG_API_KEY'])
        player_collection = extractor.players('pc-na', 'shroud,Wadu')
        expect(player_collection.players[0].name).to eq 'shroud'
        expect(player_collection.players[1].name).to eq 'Wadu'
      end
    end
